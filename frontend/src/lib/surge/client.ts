...
import type { Difficulty, GameState, Move, WinReason } from "./types";
import {
...
// The live engine has no precomputed per-tier tables the way the Python
// backend does (nothing is shipped, the table starts genuinely empty), so
// difficulty can't separate skill the same way. It still has to do
// *something* real with the difficulty picker rather than silently
// ignoring it -- varying simulation count per move, using the same
// per-tier counts the backend settled on, is the one lever available.
const LOCAL_SIMULATIONS: Record<Difficulty, number> = {
  easy: 30,
  medium: 80,
  hard: 200,
};
...
// One shared table regardless of difficulty -- there's no precomputed
// per-tier skill to keep separate, only this installation's own accrued
// play, and chrome.storage.local's 5MB quota (no unlimitedStorage
// permission) is a better fit for one table than three.
...
function engineStateToUi(gameId: string, s: EngineGameState): GameState {
  ...
  return {
    ...
    // The live engine has no precomputed per-tier tables -- difficulty only
    // ever varies simulation count (see LOCAL_SIMULATIONS) -- but the field
    // is still real and displayed in the UI, so it reports whatever
    // difficulty this specific game was actually created with.
    difficulty: localGameDifficulty.get(gameId) ?? "hard",
  };
}
...
const localGameDifficulty = new Map<string, Difficulty>();

async function createGameLocal(difficulty: Difficulty): Promise<GameState> {
  ...
  localGameDifficulty.set(gameId, difficulty);
...
async function requestAgentMoveLocal(
  ...
  const agent = await ensureLocalAgent();
  agent.nSimulations = LOCAL_SIMULATIONS[localGameDifficulty.get(gameId) ?? "hard"];
...
export async function createGame(difficulty: Difficulty = "hard"): Promise<GameState> {
  if (MODE === "mock") return mockCreateGame(difficulty);
  if (MODE === "local") return createGameLocal(difficulty);
